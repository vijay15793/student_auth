class User < ActiveRecord::Base
  ...
  devise :database_authenticatable, :recoverable, :rememberable, :trackable, :registerable
     ...
     validates :std, :length => { :maximum => 5}
     validates :section, :length => { :maximum => 5 } ,:format =>{:with=>/^[a-zA-Z]{1}$/}
     validates :mobile, :length => { :maximum => 10 }, :format => /^[0-9]{10}$/
     #validates :password, :presence => true, :length => { :within => 5..25 }
    
  # Setup accessible (or protected) attributes for your model
  attr_accessible :name,:username,:std,:section,:mobile,:email, :password, :password_confirmation, :remember_me,:role_ids,:deleted_flag
